...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description='Implements kmeans clustering using scipy.'
    )
    parser.add_argument(
...
def main() -> None:
    ...
    model = scipy.cluster.vq.kmeans2

    tic = time.perf_counter()
    for _ in range(args.num_trials):
        means, _ = model(data, args.num_clusters, iter=args.max_iter, minit='points')
